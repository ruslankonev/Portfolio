...
class Controller extends \yii\base\Controller
{
    ...
    {
        $return_url = Yii::$app->getUser()->getReturnUrl($defaultUrl);
        if($ref = Yii::$app->request->get("ref"))
        {
            //происходит ошибка при авторизации с ref и с en версией. Неправильно обрабатывается роутинг.
            //Поэтому проверяем если язык не дефолтный то ставим '/' перед get параметрами
            if (Yii::$app->language != Yii::$app->getModule('languages')->default_language)
                $return_url .= "/?ref=".$ref;
            else
                $return_url .= "?ref=".$ref;
        }
        return Yii::$app->getResponse()->redirect($return_url);
    }
