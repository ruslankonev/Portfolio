...
class Mailer extends Component
{
    ...
    /**
    *   Отправка письма об изменении статуса транзакции
    *
    *   @param User  $user
    *   @param Token $token
    *
    *   @return bool
    */
    public function sendNotify(User $user, $appname,$status_transaction)
    {
        return $this->sendMessage(
            $user->email,
            \Yii::t('app', 'Your account transaction has been completed'),
            'notify_balance',
            ['appname' => $appname,'status_transaction'=>$status_transaction]
        );
    }

    public function sendSimpleMessageWithView($email,$subject,$view,$appname,$status)
    {
        return $this->sendMessage(
            $email,
            $subject,
    ...
    public function sendSimpleMessage($email,$subject,$message,$appname)
    {
        return $this->sendMessage(
            $email,
            $subject,
            'simple',
            ['appname' => $appname,'message'=>$message]
        );
    }
    /**
     * @param string $to
