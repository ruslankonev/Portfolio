 ...
 * @property integer date_addwhitelist 
 *
 * Defined relations:
...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'btc_address'       => \Yii::t('user', 'Bitcoin Address'),
            'ref_link'          => \Yii::t('user', 'Referral link'),
            'date_addwhitelist' => \Yii::t('user', 'Date_addwhitelist'),
        ];
    }
    ...
    public function scenarios()
    {
        ...
        return ArrayHelper::merge($scenarios, [
            'register' => ['username', 'email', 'password', 'ref_link'],
            'connect'  => ['username', 'email'],
            'create'   => ['username', 'email', 'password', 'ref_link'],
            'update'   => ['username', 'email', 'password', 'eth_address'],
            'settings' => ['username', 'email', 'password', 'eth_address'],
            'ether'    => ['eth_address','date_addwhitelist'],
        ]);
    }
    ...
    public function create()
    {
        ...
        try {
            ...
            $this->ref_link = $this->generateReflink();
    ...
    public function attemptConfirmation($code)
    {
        ...
        if($token instanceof Token && !$token->isExpired)
        {
            ...
            if(($success = $this->confirm()))
            {
                ...
                {
                    $link = \klisl\languages\UrlManager::className()->createUrl([
                        'ref' => \Yii::$app->request->get("ref")
                    ]);
                }

                $message = \Yii::t('user', 'Thank you, registration is now complete.<br>Please follow the <a href="{link}">link</a> and  authenticate in to your account using the data received from email.', [
                    'link' => $link
                ]);
            }
            else
    ...
    /**
     *  Генерируем ссылку для реферальной программы
     **/
    public function generateReflink()
    {
        // try to part of email
        $ref_link = explode('@', $this->email)[0];
        $this->ref_link = md5(md5($ref_link.uniqid()));
