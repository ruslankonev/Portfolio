...
class SettingsForm extends Model
{
    ...
    {
        return [
            'usernameRequired' => [['username','eth_address'], 'required'],
            'usernameTrim' => ['username', 'filter', 'filter' => 'trim'],
            'usernameLength'   => ['username', 'string', 'min' => 3, 'max' => 255],
            'usernamePattern' => ['username', 'match', 'pattern' => '/^[-a-zA-Z0-9_\.@]+$/'],
            'eth_addressPattern' => ['eth_address', 'match', 'pattern' => '/^(0x)?[0-9a-f]{40}$/i','message'=>Yii::t('user', 'Ethereum address not valid')],
            ...
            'newPasswordValidate'=>[['current_password','new_password'],function ($attr){
                if (strlen($this->new_password)>0){
                    if (strlen($this->current_password)>0){
                        if (!Password::validate($this->current_password, $this->user->password_hash)) {
    ...
    public function attributeLabels()
    {
        return [
            ...
            'eth_address'    => \Yii::t('user', 'Ethereum Address'),
        ];
    }
    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            // Если изменился, то проверяем его наличие в WL
            if($this->user->eth_address !== $this->eth_address)
            {
                // Обновляем адрес
                $eth_address = (string) $this->eth_address;
                $this->user->eth_address = $eth_address;

                // Проверяем адрес в WL
                if(\app\models\Contract::isWhitelisted($eth_address))
                {
                    // Если адрес нашёл
                    // Меняем флаг проверки
                    $this->user->is_whitelisted = 1;
                }
                else
                {
                    // Не нашёл, то добавляем адрес
                    // флаг true - проверка с добавлением
                    \app\models\Contract::isWhitelisted($eth_address, true);
                    $this->user->is_whitelisted = 0;
                }

                $this->user->date_addwhitelist = time();
            }
    ...
    /**
    *   Saves EtherWallet.
    *   Проверка адреса в WhiteList смарт контракта
    *
    *   @return bool
    */
    public function saveEtherWallet()
    {
        $this->user->scenario = 'ether';
