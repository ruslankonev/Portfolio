...
class SecurityController extends Controller
{
    ...
    {
        //проверяем есть ли реф ссылка, если да то устанавливаем куки
        if ((\Yii::$app->request->get("ref") !== null))
            setcookie( "ref", \Yii::$app->request->get("ref"), time()+(60*60*24*30));
        return [
            'auth' => [
    ...
    {
       $ref =  \Yii::$app->session->getFlash('ref');
        ...
        if ($account->user instanceof User) {
            ...
            if (isset($_COOKIE["ref"])){
                //проверяем есть ли get параметры в текущем урле. если да то добавляем с '&', если нет то с '?'
                if (strpos($this->action->successUrl,"?") !== false)
                    $this->action->successUrl  .= '&ref='.$_COOKIE["ref"];
                else
                    $this->action->successUrl  .= '?ref='.$_COOKIE["ref"];
                //удаляем из кук ref ссылку
                setcookie('ref', $_COOKIE["ref"], time()-3600);
            }


        //if (\Yii::$app->session->hasFlash('ref'))
      //      $this->action->successUrl .= '?ref='.$ref;
