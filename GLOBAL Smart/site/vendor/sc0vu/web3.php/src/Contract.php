...
class Contract
{
    ...
    public function send()
    {
        if (isset($this->functions)) {
            ...
            {
                if(isset($arguments[0]["from"]))
                {
                    $from = $arguments[0]["from"];

                    if (AddressValidator::validate($from) === false) {
                        throw new InvalidArgumentException('Please make sure address "From" is valid.');
                    }

                    unset($arguments[0]["from"]);
                }

                $arguments = array_merge($arguments[0]);
            }
    ...
    public function call()
    {
        if (isset($this->functions)) {
            ...
            if (count($arguments) > 0)
            {   
                if(isset($test) && isset($arguments[0]))
                {
                    $transaction[] = $arguments[0];
                }
                else
